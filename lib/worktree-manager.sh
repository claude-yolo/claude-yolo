...
wt_state_file() {
    echo "$(log_dir)/claude-yolo-wt-${1}.state"
}

wt_done_marker() {
    local session="$1" index="$2"
    echo "$(log_dir)/claude-yolo-done-${session}-${index}"
}
...
wt_cleanup() {
    ...
    # Remove done markers and state file
    rm -f "$(log_dir)/claude-yolo-done-${session}-"* 2>/dev/null
    rm -f "$state_file"
