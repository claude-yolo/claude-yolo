#!/usr/bin/env bash
# control-pane.sh - Interactive control window for claude-yolo sessions
set -uo pipefail
...
NEXT_QUEUE_ID=1
TAIL_PID=""
CONTROL_SUBMIT_DELAY="${CLAUDE_YOLO_CONTROL_SUBMIT_DELAY:-0.2}"
CONTROL_PLAN_PASTE_GRACE="${CLAUDE_YOLO_CONTROL_PLAN_PASTE_GRACE:-0.5}"
CONTROL_MULTILINE_PASTE_DELAY="${CLAUDE_YOLO_CONTROL_MULTILINE_PASTE_DELAY:-0.8}"
...
control_read_continuation_line() {
    local __line_var="$1" prompt="${2:-claude-yolo...> }"
    local __line_value
...
control_plan_paste_enabled() {
    case "${CLAUDE_YOLO_CONTROL_PLAN_PASTE:-auto}" in
...
control_read_line() {
    ...
    if [[ -t 0 ]]; then
        if ! IFS= read -e -r -p "claude-yolo> " __line_value; then
            return 1
        fi
...
control_send_text_to_target() {
    ...
    buffer="claude-yolo-paste-$$-$RANDOM"
...
# Heuristic agent-idle detection. claude-yolo's agent-1 runs Claude Code; the
# idle state shows the input prompt (`>`/`❯`) at the bottom of the pane and no
# active spinner/permission prompt above it.
control_agent_prompt_line_visible() {
...
control_agent_idle_visible() {
    local content="$1"
    local tail_content last_nonempty busy_re

    tail_content="$(echo "$content" | tail -n 25)"
    busy_re='(esc to interrupt|disabled while a task is in progress|Would you like to|Do you want to proceed|Allow[[:space:]]+Deny|Showing detailed transcript|^[[:space:]]*●[[:space:]]+[A-Z][A-Za-z]*\(|[0-9]+\.[[:space:]]*Yes)'
    ...
    control_agent_prompt_line_visible "$last_nonempty" && return 0
    control_agent_prompt_line_visible "$tail_content" && return 0
...
control_wait_for_agent_idle() {
    ...
    while tmux has-session -t "$session" 2>/dev/null; do
        ...
        if control_agent_idle_visible "$content"; then
...
control_send_prompt() {
    ...
    if control_send_text_to_target "$target" "$prompt"; then
        # Claude Code can classify a burst of text plus immediate Enter as paste
...
control_start_queue() {
    ...
    if [[ "$SESSION_MODE" == "worktree" ]]; then
        echo "/queue is disabled in worktree mode because agent windows may exit after completing their assigned task."
    ...
    state_dir="$(mktemp -d "/tmp/claude-yolo-queue-${safe_session}-${queue_id}.XXXXXX")" || return 1
...
control_start_loop() {
    ...
    if [[ "$SESSION_MODE" == "worktree" ]]; then
        echo "/loop is disabled in worktree mode because agent windows may exit after completing their assigned task."
...
control_start_plan() {
    ...
    if [[ "$SESSION_MODE" == "worktree" ]]; then
        echo "/plan is disabled in worktree mode because agent windows may exit after completing their assigned task."
...
Available commands:
  ...
  /loop <interval> /queue [...] Schedule a queued loop
...
control_main() {
    ...
    echo ""
    echo "claude-yolo control ready. Type /help for commands."
